...
function prompt9() {
  // YOUR CODE HERE
  // Previous version that I am replacing with new version
  // for (let i = 0; i < starWars.length; i++) {
   // console.log(i + ". " + starWars[i]);

   starWars.forEach(function(starWars, idx) {
    console.log(idx + ". " + starWars);
   })
  }
...
// use if...else to check for odd vs. even
  if (nums.length % 2 !== 0) {
    median = nums[mid];
  } else {
...
function prompt12() {
  ...
  // outer loop with incrementer i
    for (let i = 1; i <= 10; i++) {

      // inner loop with incrementer j and requested print value
      for (let j = 11; j <= 20; j++) {
        console.log("i: $" + i + " / j: $" + j);
    }
  }
...
function prompt13() {
  ...
  // use outer loop i to iterate each array (i -> array)
  for (let i = 0; i < nestedArrays.length; i++) {

    // use inner loop j to iterate each element in the arrays (j -> element)
