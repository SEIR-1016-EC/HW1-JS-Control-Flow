...
var someNumber = Math.floor(Math.random() * (100 - 1) + 1);
...
function prompt11() {
	// nums.sort() Convert them into strings and then compare their Unicode code points.
	...
	for (let start = 0; start < nums.length - 1; start++) {
		...
		for (let next = start + 1; next < nums.length; next++) {
			if (nums[next] < nums[lower]) {
				lower = next;
			}
		}
		holder = nums[start];
		nums[start] = nums[lower];
		nums[lower] = holder;
	}
	//console.log(nums);

	//Find the median number in the following nums array (the mid point)
	let median;

	//console.log("Median in the index 12: " + nums[12]);
...
function prompt13() {
  for (let i = 0; i < nestedArrays.length; i++){
    console.log("The outer array is:", nestedArrays[i]);
    for (let k = 0; k < nestedArrays[i].length; k++){
      console.log("The inner value is:", nestedArrays[i][k]);
    }
  }
}
