...
function prompt7() {
  for(let i = 1; i<=100; i++) {
    if(i % 5 === 0) {
      ...
      newP.innerText = ("I found a " + i + ". High five!");
      let displayOutput = document.getElementById("output");
      displayOutput.appendChild(newP)
    }
  }
}
...
const max = 100
let someNumber = Math.floor(Math.random() * max); //Per MDN's documentation on Math.random()
let message;
function prompt8() {
  // print someNumber to the console in your function so you can see its value
  console.log(someNumber);
  if (someNumber < 30) {
    message = (someNumber + "? That's a small number!");
  } else if (someNumber > 29 && someNumber < 61) {
    message = (someNumber + "? The number is medium sized");
  } else {
    message = (someNumber + "? We got a big one!");
  }
