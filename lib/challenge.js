...
function prompt7() {
  for (let i = 0; i <=100; i++) {
      let isFive = i % 5 == 0
      if (isFive > 0) 
      // need to not report the 0, above logic not working as expected
        if (isFive){
          console.log(isFive, i)
...
function prompt10() {
  ...
  for (let i =1; i<100; i++){
    ...
    if (isEven){
      myArray.push(i)
      return myArray;
      // ***  getting illegal return statement error in browser
...
// NOTE:  Solution sourced from stackoverflow, unable to get it work properly:  illegal return
/* 1. Sort the numbers so that they are in order by value.
   2. Find out the median's index in the array. If the array's length is even, the median is the 
      average of the two numbers on either side of the index.
   3. For arrays of odd length, it's easy to pluck out the middle number. But for arrays of even length, 
      it's not. So, you test to find out whether your array is odd- or even-lengthed by finding out if
      dividing the array's length by two returns a whole number or not. 
      If it's a whole number, that means the array's length is even, and you have to 
      calculate the average of the two numbers on either side of the median.
*/
return median % 1 == 0 ? (nums[median - 1] + nums[median]) / 2 : nums[Math.floor(median)];
console.log(median);
}
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
// prompt11();
...
function prompt13() {
  ...
  for (let i = 0; i <= nestedArrays.length; i++) {
    for (let j = 0; j <= nestedArrays.length; j++) {
      console.log("i =" +nestedArrays[i] + " j =" + nestedArrays[j]);
    }
  }
}
