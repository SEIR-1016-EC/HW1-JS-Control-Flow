...
function prompt8() {
  ...
  if (someNumber <30) {
    ...
    console.log("The number is mediumsized.");
  } else {
...
function prompt10() {
// empty array to store even numbers
  const myArray = [];
  // Loop from 0 to 100
  for (let i = 0; i <= 100; i++) {
    // If even, push it into the array
    if (i % 2 === 0) {
    myArray.push(i);
  }
}
...
// Find the median
  ...
  if(length % 2===1) {
    ...
    const middle1 = nums[length / 2-1];
    const middle2 = nums[length / 2];
...
function prompt13() {
  //the outer loop iterates over the main array nestedArrays
  for (let i = 0; i < nestedArrays.length; i++) {
    //the inner loop iterates over each subarray within the main array.
